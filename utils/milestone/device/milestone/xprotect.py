...
from protocol.util.dataview import DataView
...
class XPMOBILE(object):
    ...
    def buildPacket(sequenceId, commandName, commandParams=None, connectionId=None):
        ...
        xmlCommunication = lxml.etree.Element('Communication', nsmap=dict(xsi=XPMOBILE.NS.XSI, xsd=XPMOBILE.NS.XSD))
        if connectionId is not None: lxml.etree.SubElement(xmlCommunication, 'ConnectionId').text = connectionId
        xmlCommand = lxml.etree.SubElement(xmlCommunication, 'Command')
        if sequenceId is not None: xmlCommand.attrib['SequenceId'] = str(sequenceId)
        lxml.etree.SubElement(xmlCommand, 'Type').text = commandType
        lxml.etree.SubElement(xmlCommand, 'Name').text = commandName
        xmlCommandParams = lxml.etree.SubElement(xmlCommand, 'InputParams')
        for paramKey, paramValue in commandParams.items():
            ...
            lxml.etree.SubElement(xmlCommandParams, 'Param', Name=paramKey, Value=paramValue)
        payload = lxml.etree.tostring(xmlCommunication, pretty_print=False, encoding='utf-8', xml_declaration=True)
    ...
    def parsePacket(data):
        xmlRoot = lxml.etree.fromstring(data)
