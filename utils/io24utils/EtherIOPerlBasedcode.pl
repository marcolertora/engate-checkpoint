...
#---------------------------------------------------------------
$val = &ReadPortValue($IP_Address, $Port, "a");
printf("Port A value: [0x%02x]\n", $val);
$val = &ReadPortValue($IP_Address, $Port, "b");
printf("Port B value: [0x%02x]\n", $val);
$val = &ReadPortValue($IP_Address, $Port, "c");
printf("Port C value: [0x%02x]\n", $val);

exit;

#---------------------------------------------------------------
# Make port A an output (0=output, 255=input on all bits) port
#---------------------------------------------------------------
&ConfigurePortDirection($IP_Address, $Port, "A", 0);
...
for ($i=0; $i<8; $i++)
{
  print "Sending [$i] to port A\n";
	&WritePortValue($IP_Address, $Port, "A", $i);
  `sleep 1`;
}
